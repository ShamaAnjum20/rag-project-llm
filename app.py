...
st.title("📄 Intelligent Document Q&A System with RAG")
...
top_k = st.sidebar.slider(
    ...
    max_value=5,
    value=3,
    help="Number of document chunks retrieved from vector DB"
)
...
if query:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            # -------- DOCS FOUND --------
                ...
                if answer.lower().startswith("i don't know"):
                    ...
                    for d, _ in docs:
                        ...
                        st.session_state.citations.append(src)
                        pg.execute("INSERT INTO chunk_references (source) VALUES (%s)", (src,))

            # Remove duplicates
            st.session_state.citations = list(set(st.session_state.citations))

            # -------- CONFIDENCE FUNCTION --------
            def final_confidence_score(docs, used_gk):
                if used_gk:
                    return 40
                if not docs:
                    return 20
                ...
                relevance_ratio = len([d for d, s in docs if s < 1.5]) / len(docs)

                confidence = (relevance_ratio * 0.7) + (max(0, 1 - avg_score/2) * 0.3)
                return round(confidence * 100, 2)

            confidence = final_confidence_score(docs, used_gk)
            ...
            st.sidebar.metric("Answer Confidence", f"{confidence}%")
